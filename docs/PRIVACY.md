...
| Tokens in URLs | `sanitizeUrl` removes sensitive query-string and fragment parameters, and basic-auth credentials, on navigation, network and metadata URLs. rrweb's own page URL, link and form URLs, and `url()` references in inline styles go through the same rule (`sanitizeRrwebEvent`). | SDK and server | contracts tests, E2E (`?token=` canary) |
...
| Personal identifiers via `identify()` and `annotate()` | Email-shaped ids are refused and trait keys that look sensitive are dropped in the SDK; the server drops sensitive-looking keys and scrubs values again | SDK and server | SDK tests, ingest sanitiser |
...
| Arbitrary code execution from generated tests | The runner validates the file (single Playwright import, no `require`, `import()`, `eval`, `fetch`, `page.request`, `route()`, `window[...]`, `globalThis`, `.constructor`, no `process` other than `REPRO_FIXTURE_*`, no absolute URLs outside the demo origin), runs it with `spawn` and a fixed argument list, no shell, under a timeout, only against the bundled demo app. A second pass parses the file and accepts only the syntax-tree shapes, identifiers and member names the generator emits. See `docs/REPRODUCTION_RUNNER.md`. | worker | worker unit tests (`validate.test.ts`, `ast.test.ts`) |
...
2. **Sensitive values in URL paths and bare fragments.** Only query-string parameters and `key=value` fragments are sanitised. `/reset/abcdef123` keeps its path, and a fragment such as `#abcdef123` with no `=` is kept. Use `annotate` to mark such routes and consider masking on the server side.
